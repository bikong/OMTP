       ...
       %%*************************************************************************%%
                              %%  File_name:gsaSVRsg.m  %%
                              %%  Author: Bikong        %%



function [bestMSE, bestc, bestg, gsa_option] = gsaSVRcg(train_label, train, gsa_option)
...
for iteration=1:gsa_option.max_it
    ...
    for i = 1:gsa_option.N
 		...
 		cmd = ['-v ',num2str(gsa_option.v),' -c ',num2str( X(i,1) ),' -g ',num2str( X(i,2) ),' -s 3 -p 0.01'];
