...
function [bestmse,bestc,bestg,] = gsSVMcg(train_result,train,type, gs_option)
...
for k = 1:r
    for i = 1:m
        for j = 1:n
            ...
            ' -g ',num2str( basenum^Y(i,j,k) ),' -p ',num2str( basenum^X(i,j,k) ), ' -s ',num2str(type)];
